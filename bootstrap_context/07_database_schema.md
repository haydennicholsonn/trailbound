...
This schema is intentionally practical for PHP/MySQL on shared hosting.

It can be normalized further later, but this is good for MVP.

## Database name

```sql
CREATE DATABASE trailbound CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
```
...
Optional if not using PHP native sessions only.
...
```sql
INSERT INTO regions (code, name, description, lore, distance_required_m, sort_order)
VALUES
...
```sql
INSERT INTO chests (code, name, rarity, description)
VALUES
...
```sql
INSERT INTO items (code, name, item_type, rarity, description)
VALUES
...
```sql
INSERT INTO skill_nodes (code, name, branch, description, effect_type, effect_value, cost, position_x, position_y)
VALUES
...
- `activities(provider, provider_activity_id)` unique
- `user_quests(user_id, status)`
- `notifications(user_id, read_at)`
- `strava_webhook_events(processed_at, created_at)`

Do not skip these once data grows.
